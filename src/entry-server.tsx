...
// Direct (non-lazy) page imports required for SSR renderToString
...
export function getStaticRoutes(): string[] {
  ...
  const works = worksJson as unknown as ELMSWork[];
  const workRoutes = works.map((work) => `/${work.workInformation.workId}`);
...
export function render(url: string, base = ''): { html: string } {
  ...
  const siteJsonLd = {
    ...
    url: base || import.meta.env.BASE_URL || '/',
    inLanguage: 'English',
  };
  ...
  const html = renderToString(
    <HelmetProvider context={helmetContext}>
      <Helmet>
        ...
        <meta name="keywords" content="e-lit, electronic literature, new media, electronic literature organization, digital media" />
